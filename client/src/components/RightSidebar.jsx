...
const RightSidebar = ({ info }) => {
  ...
  return (
    <div
      className={`${!info ? "hidden" : "show"} w-120 flex h-full bg-zinc-900 text-white flex-col border-l-2 border-purple-900 shrink-0`}
    >
      ...
      <div className="flex flex-col w-full text-center pt-2">
        ...
        <div className="flex overflow-y-scroll max-h-120 pt-4 flex-wrap gap-4 justify-center">
          {msgImages.map((image)=><img
            src={image}
            className="size-25 object-cover bg-purple-800 rounded-xl"
          />)}
        </div>
      </div>
    </div>
